...
from storeapi.tasks import APIResponseError, send_email, _generate_cute_creature_api, generate_and_add_to_post
...
@pytest.mark.anyio
async def test_send_message_api_error(mock_httpx_client):
...
async def test_generate_cute_creature_api_server_error(mock_httpx_client):
    ...
    with pytest.raises(APIResponseError,match="API request failed with status code 500"):
        await _generate_cute_creature_api(prompt="A cat")    
